...
impl<K: KernelDevOp> Ext4BlockWrapper<K> {
    pub fn new(block_dev: K::DevType) -> Result<Self, i32> {
        Self::new_with_mount(block_dev, "/", "ext4_fs")
    }
    ...
    pub fn new_with_mount(
        ...
        let c_name = CString::new(device_name).map_err(|_| EINVAL as i32)?;
    ...
    pub extern "C" fn dev_bread(
        ...
        unsafe {
            ...
            let buffer = from_raw_parts_mut(buf as *mut u8, buf_len);
    ...
    pub extern "C" fn dev_bwrite(
        ...
        unsafe {
            ...
            let buf_len = ((*(*bdev).bdif).ph_bsize * blk_cnt * 1) as usize;
            let buffer = from_raw_parts(buf as *const u8, buf_len);
