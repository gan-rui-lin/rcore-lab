...
impl IrqController for QemuIrqController {
    ...
    fn claim() -> Option<u32> {
        warn!(
            "[board] LoongArch64 IRQ handler: external interrupt received but not yet dispatched"
        );
        None
    }
...
impl BoardDevices for QemuBoardDevices {
    fn init_devices() {
        info!(
            "[board] LoongArch64 QEMU virt: device_init (PCI block device will be probed lazily)"
        );
    }
