...
/// PLIC (Platform-Level Interrupt Controller) MMIO accessor.
pub struct PLIC {
    base_addr: usize,
}

#[derive(Copy, Clone)]
/// Interrupt target privilege level.
pub enum IntrTargetPriority {
    /// Machine mode target.
    Machine = 0,
    /// Supervisor mode target.
    Supervisor = 1,
}

impl IntrTargetPriority {
    /// Number of supported priority targets.
...
impl PLIC {
    ...
    /// Create a new PLIC accessor at the given MMIO base.
    ...
    /// Enable an interrupt source for a given hart/target.
    pub fn enable(
        &mut self,
        hart_id: usize,
        target_priority: IntrTargetPriority,
    ...
    #[allow(unused)]
    /// Disable an interrupt source for a given hart/target.
    pub fn disable(
        &mut self,
        hart_id: usize,
        target_priority: IntrTargetPriority,
    ...
    /// Set interrupt threshold for a given hart/target.
