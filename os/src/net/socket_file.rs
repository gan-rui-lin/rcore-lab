...
use super::{poll_net, with_net_stack_write};
...
impl File for SocketFile {
    ...
    fn poll(&self, events: PollEvents) -> PollEvents {
        poll_net();
        let result = with_net_stack_write(|stack| {
            ...
            match self.sock_type {
                SocketType::Tcp => {
                    ...
                    if events.contains(PollEvents::POLLIN)
                        && (socket.can_recv() || (was_connected && !socket.may_recv()))
                    {
        ...
        result.unwrap_or_else(PollEvents::empty)
    }
...
impl SocketFile {
    fn tcp_read(&self, mut user_buf: UserBuffer) -> usize {
        loop {
            ...
            let result = with_net_stack_write(|stack| {
                ...
                if self.nonblock {
                    return Some(usize::MAX); // -EAGAIN encoded
                }
    ...
    fn tcp_write(&self, user_buf: UserBuffer) -> usize {
        loop {
            ...
            let result = with_net_stack_write(|stack| {
                ...
                if !socket.may_send() {
                    return Some(0); // Connection closed
                }
                    ...
                    match socket.recv_slice(&mut tmp[..]) {
                        Ok((n, _endpoint)) => {
                            ...
                            for slice in user_buf.buffers.iter_mut() {
                                ...
                                slice[..copy_len]
                                    .copy_from_slice(&tmp[offset..offset + copy_len]);
                                offset += copy_len;
                            }
                            return Some(offset);
                        }
    ...
    fn udp_write(&self, user_buf: UserBuffer) -> usize {
        ...
        poll_net();
        let (bytes, need_poll) = match with_net_stack_write(|stack| {
...
impl Drop for SocketFile {
    fn drop(&mut self) {
        ...
        let _ = with_net_stack_write(|stack| match self.sock_type {
            SocketType::Tcp => {
                let socket = stack.sockets.get_mut::<tcp::Socket>(self.handle);
