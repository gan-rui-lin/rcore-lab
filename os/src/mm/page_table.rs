...
impl PageTable {
    ...
    pub fn map(&mut self, vpn: VirtPageNum, ppn: PhysPageNum, flags: PTEFlags) {
        ...
        // Allow overwriting an existing PTE (needed for MAP_FIXED which
        // remaps pages that were previously mapped by the ELF loader).
        if pte.is_valid() {
            log::trace!("[page_table] overwriting vpn {:?} (old ppn {:?})", vpn, pte.ppn());
        }
