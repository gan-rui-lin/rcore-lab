...
pub fn exec(path: &str, args: &[*const u8]) -> isize {
    ...
    let argv = if args.is_empty() {
        argv_buf = [core::ptr::null()];
        &argv_buf[..]
    } else {
        args
    };
    let envp_buf = [core::ptr::null()];
    sys_exec(path, argv, &envp_buf)
}
...
pub fn execve(path: &str, args: &[*const u8], envp: &[*const u8]) -> isize {
    ...
    let envp_buf;
    let envp = if envp.is_empty() {
        envp_buf = [core::ptr::null()];
        &envp_buf[..]
    } else {
        envp
    };
...
pub fn dup3(oldfd: usize, newfd: usize) -> isize {
    sys_dup3(oldfd, newfd)
}
