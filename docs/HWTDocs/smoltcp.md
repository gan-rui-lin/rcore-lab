...
*smoltcp* does not need heap allocation *at all*, is [extensively documented](https://docs.rs/smoltcp/), and compiles on stable Rust 1.80 and later.

*smoltcp* achieves [~Gbps of throughput](https://docs.rs/crate/smoltcp/0.12.0#examplesbenchmarkrs) when tested against the Linux TCP stack in loopback mode.
...
The `log` feature enables logging of events within the networking stack through the [log crate](https://crates.io/crates/log). Normal events (e.g. buffer level or TCP state changes) are emitted with the TRACE log level. Exceptional events (e.g. malformed packets) are emitted with the DEBUG log level.

This feature is enabled by default.

### Feature defmt

The `defmt` feature enables logging of events with the [defmt crate](https://crates.io/crates/defmt).
...
### Features phy-raw\_socket and phy-tuntap\_interface
...
Enable [IPv4](https://tools.ietf.org/rfc/rfc791.txt), [IPv6](https://tools.ietf.org/rfc/rfc8200.txt) and [6LoWPAN](https://tools.ietf.org/rfc/rfc6282.txt) respectively.
...
### ASSEMBLER\_MAX\_SEGMENT\_COUNT
...
All examples provide a `--pcap` option that writes a [libpcap](https://wiki.wireshark.org/Development/LibpcapFileFormat) file containing a view of every packet as it is seen by *smoltcp*.
...
Read its [source code](https://docs.rs/examples/tcpdump.rs), then run it as:
...
Read its [source code](https://docs.rs/examples/httpclient.rs), then run it as:
...
Read its [source code](https://docs.rs/examples/ping.rs), then run it as:

```sh
cargo run --example ping -- --tap tap0 ADDRESS
```

It sends a series of 4 ICMP ECHO\_REQUEST packets to the given address at one second intervals and prints out a status line on each valid ECHO\_RESPONSE received.

The first ECHO\_REQUEST packet is expected to be lost since arp\_cache is empty after startup; the ECHO\_REQUEST packet is dropped and an ARP request is sent instead.
...
Read its [source code](https://docs.rs/examples/server.rs), then run it as:
...
Read its [source code](https://docs.rs/examples/client.rs), then run it as:
...
Read its [source code](https://docs.rs/examples/benchmark.rs), then run it as:
...
A typical result (achieved on a Intel Core i5-13500H CPU and a Linux 6.9.9 x86\_64 kernel running on a LENOVO XiaoXinPro 14 IRH8 laptop) is as follows:
...
Read its [source code](https://docs.rs/examples/loopback.rs), then run it without `std`:
...
It opens a server and a client TCP socket, and transfers a chunk of data. You can examine the packet exchange by opening `loopback.pcap` in [Wireshark](https://wireshark.org/).
...
*examples/loopback\_benchmark.rs* is another simple throughput benchmark.

Read its [source code](https://docs.rs/examples/loopback_benchmark.rs), then run it as:
...
See [LICENSE-0BSD](https://docs.rs/crate/smoltcp/LICENSE-0BSD.txt) for details.
