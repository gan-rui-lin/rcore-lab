...
pub unsafe fn emulate_load_store_insn(pt_regs: &mut TrapFrame) {
    ...
    let inst: u32 = *(pt_regs.sepc as *const u32);
    la_inst = inst;

    rd = ((la_inst >> 5) & 0x1f) as usize;
    addr = badv::read().raw() as u64;

    match la_inst & 0x3f {
        LDH_OP => {
            res = unaligned_read(addr, &mut value, 2, 1);
            pt_regs.x[rd] = (value as i64) as usize;
        }
        LDHU_OP => {
            res = unaligned_read(addr, &mut value, 2, 0);
            pt_regs.x[rd] = value as usize;
        }
        LDW_OP => {
            res = unaligned_read(addr, &mut value, 4, 1);
            pt_regs.x[rd] = (value as i32) as usize;
        }
        LDWU_OP => {
        ...
        STH_OP => {
            res = unaligned_write(addr, pt_regs.x[rd] as u64, 2);
        }
        STW_OP => {
            res = unaligned_write(addr, pt_regs.x[rd] as u64, 4);
        }
        STD_OP => {
            res = unaligned_write(addr, pt_regs.x[rd] as u64, 8);
        }
        _ => {
            match la_inst & 0xffff {
                LDXH_OP => {
                    res = unaligned_read(addr, &mut value, 2, 1);
                    pt_regs.x[rd] = (value as i16) as usize;
                }
                LDXHU_OP => {
                    res = unaligned_read(addr, &mut value, 2, 0);
                    pt_regs.x[rd] = value as usize;
    ...
    if res != 0 {
        panic!("Unhandled unaligned access: inst={:#x} addr={:#x}", la_inst, addr);
    }
